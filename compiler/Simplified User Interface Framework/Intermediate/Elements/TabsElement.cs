	...
	public class TabsElement : Element
	{
		...
		protected override void OnLoad()
		{
			...
			for(int i = 0; i < Tabs.Length; i++)
			{
				...
				if(tab.Type == ElementType.Separator)
				{
					// Only create list item for the separator
					var listItem = AddListItem($"{(isHorizontal ? "horizontal-separator" : "vertical-separator")}.tab-list-separator");
					ApplyClass(listItem);
					listItem.InlinedStyles = new Dictionary<string, string>();
					// listItem.InlinedStyles["display"] = "block";
					if(isHorizontal)
					{
						// TODO:: Add a default style for horizontal / vertical, so we can remove the hard override here
						listItem.InlinedStyles["width"] = "100%";
						listItem.InlinedStyles["height"] = "1px";
					}
					else
					{
						// TODO:: Add a default style for horizontal / vertical, so we can remove the hard override here
						listItem.InlinedStyles["width"] = "1px";
						listItem.InlinedStyles["height"] = "100%";
					}
					CopyStyles(tab, listItem);
				}
				else
				{
					var key = tab.Value ?? i.ToString();
					var name = tab.Configurations?["name"] ?? key;

					// Create list
					var listItem = AddListItem($"button.tab-selector onclick(\"\") = {name}");
					ApplyClass(listItem);


					// Create content
					// var contentItem = tableContentHolder.AddChild(tab.Source);
					// ApplyClass(contentItem);
				}
			}
			*/
		}
